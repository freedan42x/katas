...
class Functor f => Pretty f where
  prettyExpr :: Pretty g => f (Expr g) -> String

pretty :: Pretty f => Expr f -> String
pretty (In e) = prettyExpr e

instance Pretty Lit where
  prettyExpr (Lit n) = show n

instance Pretty Add where
  prettyExpr (Add a b) = "(" ++ pretty a ++ "+" ++ pretty b ++ ")"

instance Pretty Mult where
  prettyExpr (Mult a b) = "(" ++ pretty a ++ "*" ++ pretty b ++ ")"

instance (Pretty f, Pretty g) => Pretty (f :+: g) where
  prettyExpr (Inl l) = prettyExpr l
  prettyExpr (Inr r) = prettyExpr r
